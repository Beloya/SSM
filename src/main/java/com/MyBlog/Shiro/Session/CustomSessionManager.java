...
public class CustomSessionManager {
		...
		for (Session session : sessions) {
			...
			if(null != obj && obj instanceof SimplePrincipalCollection){
				...
				if(null != obj && obj instanceof Users){
					...
					//比较用户ID，符合即加入集合
					if(null != user && idset.contains(user.getU_Id())){
						list.add(spc);
					}
				}
			}
	...
	private UserOnline getSession(Session session){
		...
		if(obj instanceof SimplePrincipalCollection){
			...
			if(null != obj && obj instanceof Users){
				//存储session + user 综合信息
				UserOnline userBo = new UserOnline((Users)obj);
				//最后一次和系统交互的时间
	...
	public static String getSESSION_STATUS() {
		return SESSION_STATUS;
	}
	public static void setSESSION_STATUS(String sESSION_STATUS) {
