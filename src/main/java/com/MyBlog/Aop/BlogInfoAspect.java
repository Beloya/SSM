...
public class BlogInfoAspect {
	...
	public void before(JoinPoint call) {
		...
		/*
		 * String className = call.getTarget().getClass().getName(); String methodName =
	...
	public void afterReturn(JoinPoint call) {
		 ...
		 * RequestContextHolder.getRequestAttributes()).getRequest(); String className =
		 * call.getTarget().getClass().getName(); String methodName =
		 * call.getSignature().getName();
		 */
	...
	public void after(JoinPoint call) {
		...
		case "NewarchivesCommit":
			...
			html.append("有人给你评论了");
			html.append(archivescommit.getCreatedBy() + " 在编号为" + archivescommit.getAid() + "文章说了:"
					+ archivescommit.getContext());
			break;
		case "NewSay":
		...
		email.setSubject("博客有人类来啦").setText(html.toString()).setTomail("468501955@qq.com");
	...
	public void afterThrowing(JoinPoint call, Throwable ex) {
		...
		email.setSubject("系统发生异常，请检查").setText(html.toString()).setSenddate(new Date()).setTomail("468501955@qq.com");
		new QQEmailUtilFactory().CreateEmail().JoinEmailQueue(email);
