										...
										<div class="box-body" id="layer-photos-demo">
											...
											<table id="example1" class="table table-bordered table-striped">
												...
												<tbody>
													...
													<c:forEach items="${bbs_comments}" var="list">
														<tr>
															<td class="">${list.userName}</td>
															<td>${list.comment_content}</td>
							
															<td><fmt:formatDate value="${list.comment_time}"
...
$("#bbs_commentManage").addClass("active");
$("#bbs_commentManage").addClass("active");
...
, tab: function(pic, layero){
  ...
  function deletedata(id){
	  ...
	  layer.confirm('确认删除?', {icon: 3, title:'提示'}, function(index){
		 ...
		 $.ajax({
			 url:"<%=request.getContextPath() %>/forumfile/bbs_commentdel.html",
					 type:'post',
					 data:"id="+id,
					 async:false,
					 success:function(data) {
		   		    	   if(data.status==0){
		   		    		layer.msg(data.msg, {icon:6, time:500}, function(index){
		   		    			location.reload();
			   		   		});
					 }
