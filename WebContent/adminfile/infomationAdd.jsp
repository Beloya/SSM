...
<body class="skin-blue sidebar-mini">
  ...
  <div class="col-xs-8 col-md-6" style="padding-left:0px;">
    <a href="#" class="thumbnail">
     <img data-src="" alt="" src="<%=request.getContextPath() %>/" id="showImg" data-holder-rendered="true" style="height: 300px; width: 100%; display: block;" />
     <input type="text" name="Img" id="img" class="layui-hide" value="" />
    </a>
  </div>
...
$("#infoMenu").css("display","block");
  ...
  layui.use(['element','form', 'layedit', 'laydate','upload','layer'], function(){
   ...
   layedit.set({
	  ...
	  function Success() {
			layer.msg("发布成功", {icon:6, time:2000}, function(index){
	        layer.close(index);
	  ...
	  function Error() {
			layer.msg("发布失败", {icon:5, time:2000}, function(index){
	      layer.close(index);
	  });
		}
	  form.on('submit(*)', function(data){
		  ...
		  //记得返回msg数据
	 $.post('<%=request.getContextPath() %>/adminfile/infomation/add.html', data.field, function(msg) {
