 ...
 <script src="<%=request.getContextPath() %>/js/autoSave.js" type="text/javascript"></script>
    ...
    layui.use(['form', 'layedit', 'laydate'], function(){
    	  ...
    	  form.on('select(typeSelect)', function(data){
			...
			$("#FormBtn").addClass("layui-btn-disabled");
 		    $.ajax({ 
 		    	...
 		    	url: "<%=request.getContextPath()%>/Createarchives.html", 
 		    	</c:if>
 		   	<c:if test="${!empty archives}">
		    	url: "<%=request.getContextPath()%>/Updatearchives.html", 
		    	</c:if>
 		    	type:'POST',
 		    	data:data.field,
     ...
     $("#NewType").click(function(){
        ...
        $.ajax({ 
	    	url: "<%=request.getContextPath()%>/NewType.html", 
	    	type:'POST',
...
　　　　 	 layer.msg('添加完成', {
 	...
 	var loadindex = layer.load(2, {time: 10*1000}); 
     $.ajax({ 
	    	url: "<%=request.getContextPath()%>/NewFlag.html", 
	    	type:'POST',
...
var tmp;
       ...
       editor.customConfig.uploadImgTimeout = 30000;
        editor.customConfig.uploadImgServer = '<%=request.getContextPath()%>/upload.html';
